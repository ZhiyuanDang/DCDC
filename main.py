...
def main():
    ...
    logger.info('Best evaluation results at epoch %d are: ACC: %.3f' % (best_epoch, best_acc))
    logger.info('Done')
...
def train_head(epoch, net, hidx, head, otrainset, ptrainset, optimizer, criterion, writer):
    ...
    for batch_idx, (obatch, pbatch) in enumerate(itertools.izip(oloader, ploader)):
        ...
        # forward
        x_o, ologits = net(oinputs)
        ologits = ologits[hidx]
        
        x_p, plogits = net(pinputs)
        plogits = plogits[hidx]
        
        #b = torch.rand(x_o.repeat(cfg.data_nrepeat, 1).shape[0],1).cuda()
        #x_immi = b * x_o.repeat(cfg.data_nrepeat, 1) + (1 - b) * x_p
        #_, x_immi_outs = net(x_immi, trunk = 0)
        #x_immi_outs = x_immi_outs[hidx]
        
        loss = criterion(net, ologits.repeat(cfg.data_nrepeat, 1), plogits, head)
